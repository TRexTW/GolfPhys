## 高爾夫物理學之視覺語言模型
* 提示詞工程
* 檢索增強生成
* 人工智慧代理
* 視覺語言模型優化

不同深度的人工智慧應用

#高爾夫物理學 #人工智慧 #大語言模型

訓練大語言模型，可以使用多模態資料，包含文字、聲音、圖片、影片等。如果是影像資料，可稱為視覺語言模型。

1. #提示詞工程 給AI看 Golf 小抄
2. #檢索增強生成 給 AI 查 Golf 圖書館
3. #人工智慧代理 給 AI 安排每天工作
4. #視覺語言模型優化 給 AI 進修 Golf
   
#AI教育
#小學生也能訓練AI模型
#YOLO #VLM

YOLO Golfer Pose Estimation Model。

![YOLO Golfer Pose Estimation Model。](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Golfer-Pose-Estimation-Golfer-Person.gif)

YOLO Golfer Pose Estimation Model wtih OBS 3 cameras 。

![YOLO Golfer Pose Estimation Model  wtih OBS 3 cameras 。](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Pose-Estimation-Golf-Person-3camera.gif)

YOLO Pose Estimation Model - Person 。
![YOLO Pose Estimation Model - Person](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Pose-Estimation-Golf-2-Person-CCU.PNG)


YOLO Pose Estimation Model - Person with OBS 3 cameras 。
![YOLO Pose Estimation Model - Personwith OBS 3 cameras 。](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Pose-Estimation-Golf-3-Person-3-Camera-CCU.PNG)


YOLO Pose Estimation Model - Person with OBS 2 cameras for example 。
![YOLO Pose Estimation Model - Personwith OBS 2 cameras for example 。](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Pose-Estimation-Golf-Anne-Lee-2-Camera-CCU.gif)

YOLO Golfer Pose Estimation Model - YOLO-Golfer-Pose-Estimation-Model-Home-simple-golf-driving-range
![YOLO Golfer Pose Estimation Model](https://github.com/TRexTW/GolfPhys/blob/main/figure/YOLO-Golfer-Pose-Estimation-Model-Home-simple-golf-driving-range.gif)


Chiayi Taibao golf-driving-range 2014.08.20
![YOLO Golfer Pose Estimation Model](https://github.com/TRexTW/GolfPhys/blob/main/figure/Chiayi-Taibao-Golf-Driving-Range-201408220.JPG)





